...
class Player extends PIXI.Container{constructor(t){super(),this.id=t.id,this.name=t.name,this.destination={x:this.x,y:this.y},this.moving=!1,this.dx=0,this.dy=0,this.abilities=[],this.abilities.length=4,this.animation={yMax:6,speed:.013},this.character=new PIXI.Sprite(PIXI.Texture.from(game.data.characters.default)),this.addChild(this.character),this.nicknameSprite=new BorderText(this.name),this.addChild(this.nicknameSprite),this.updateInfo(t)}get worldPos(){return{x:this.sector.x*game.info.SECTOR_SIZE+this.x,y:this.sector.y*game.info.SECTOR_SIZE+this.y}}getSector(){return game.world.sectors[this.sectorName]}getWorldBounds(){return new PIXI.Rectangle(this.worldPos.x,this.worldPos.y,this.width,this.height)}getSectorBounds(){return new PIXI.Rectangle(this.x,this.y,100,100)}async updateInfo(t){this.info=t,t.textureBlob&&(this.character.texture=PIXI.Texture.from(URL.createObjectURL(t.textureBlob)));var i=subVec({x:this.x,y:this.y},this.destination);this.map=t.map,this.x=t.x+i[0],this.y=t.y+i[1],this.sector=t.sector,this.sectorName=game.getSectorName(t.sector),this.speed=t.speed,this.dx=t.dx,this.dy=t.dy}colliding(){return!game.world.withinWorld(this.getWorldBounds())||this.getSector().colliding(this.getSectorBounds())}updateCharacter(t){this.info.textureBlob=t.textureBlob,this.character.texture=PIXI.Texture.from(URL.createObjectURL(t.textureBlob))}updateFloatAnimation(){var t=this.animation.speed;this.moving&&(t*=this.speed);var i=performance.now()*t/this.animation.yMax;this.character.y=(Math.sin(i)-1)*this.animation.yMax}update(){if(this.moving=0!=this.dx||0!=this.dy,this.moving){var t=this.x,i=this.y;this.x+=this.dx*this.speed,this.y+=this.dy*this.speed,this.colliding()&&(this.x=t,this.y=i)}centerTo(this.nicknameSprite,this),this.nicknameSprite.y=0,this.updateFloatAnimation()}moveBy(t){t.x&&(this.info.x=this.x=t.x),t.y&&(this.info.y=this.y=t.y),isNaN(t.dx)||(this.info.dx=this.dx=t.dx),isNaN(t.dy)||(this.info.dy=this.dy=t.dy)}moveTo(t){this.info.x=t.x,this.info.y=t.y;this.destination={x:t.x-this.width/2,y:t.y-3/4*this.height}}destroy(t){super.destroy(t),console.log(this.name,"destroyed")}}
...
class Sector extends PIXI.Container{constructor(t){super(),this.info=t,this.id=t.id||t.name,this.name=t.name,this.x=t.x,this.y=t.y,this.players={},game.data.maps[game.info.map][this.id]&&(this.data=game.data.maps[game.info.map][this.id],this.loadData()),this.data&&this.data.background?this.background=PIXI.Sprite.from(PIXI.Texture.from(this.data.background)):(this.background=PIXI.Sprite.from(PIXI.Texture.WHITE),this.background.tint=65536*t.x+256*t.y+255),this.background.width=this.background.height=game.info.SECTOR_SIZE,this.addChild(this.background),this.updatePlayers()}async loadData(){this.data.collisionMap&&(this.collision=new PixelQueryer(await game.loadFile(this.data.collisionMap)))}update(t){if(game.player){var e=game.world.worldToScreen(this.info);this.x=e.x,this.y=e.y,Object.values(this.players).forEach(e=>e.update(t))}}worldToSector(t={x:0,y:0}){return{x:t.x-this.info.x,y:t.y-this.info.y}}sectorToWorld(t={x:0,y:0}){return{x:t.x+this.info.x,y:t.y+this.info.y}}updatePlayers(){var t=this,e=game.info.players.filter(e=>t.name==e.sector.x+"-"+e.sector.y);for(var i in this.players)e.map(t=>t.id).includes(i)||this.removePlayer(i);for(var a of e){i=this.addPlayer(a);game.info.id==a.id&&(game.player=i,this.removeChild(i),this.addChild(i))}}addPlayer(t){if(t.textureBlob=blobFromBytes(t.textureBlob),this.players[t.id])return this.players[t.id].updateInfo(t),this.players[t.id];var e=new Player(t);return this.players[t.id]=e,this.addChild(e),e}removePlayer(t){this.removeChild(this.players[t]),this.players[t].destroy({children:!0}),delete this.players[t]}movePlayer(t){this.players[t.id].moveBy(t)}pointColliding(t){return this.collision&&this.collision.getPixel(t)==GameColors.green}colliding(t){var e=[{x:t.left,y:t.top},{x:t.left,y:t.bottom},{x:t.right,y:t.top},{x:t.right,y:t.bottom}],i=!1;for(var a of e)i=i||this.pointColliding(a);return i}updateCharacter(t){var e=this.players[t.id];t.textureBlob=blobFromBytes(t.textureBlob),e.updateCharacter(t)}destroy(t){super.destroy(t),console.log(this.name,"destroyed")}}
function createBinder(n){var t={};return function(n,...e){var r=t[n];return r||(r=this[n].bind(this,...e),t[n]=r),r}.bind(n)}function blobFromBytes(n){if(n)return new Blob([n],{type:"image/png"})}function isBoolean(n){return"boolean"==typeof n}function centerTo(n,t){t||(t={width:0,height:0}),n.x=t.width/2-n.width/2,n.y=t.height/2-n.height/2}function numberToColor(n){var t=n.toString(16);return"#000000".substr(0,7-t.length)+t}function lerpXY(n,t,e){return{x:n.x+e*(t.x-n.x),y:n.y+e*(t.y-n.y)}}function vec(...n){return 1==n.length?"object"==typeof n[0]?Object.values(n[0]):n[0]:n}function mag2(...n){return(n=vec(...n)).reduce((n,t)=>n+t*t,0)}function mag(...n){return Math.sqrt(mag2(...n))}function rectsIntersect(n,t){return n.x+n.width>t.x&&n.x<t.x+t.width&&n.y+n.height>t.y&&n.y<t.y+t.height}function norm(...n){n=vec(...n);var t=mag(...n);return n.map(n=>n/t)}function subVec(n,t){return vec(n).map((n,e)=>n-vec(t)[e])}function addVec(n,t){return vec(n).map((n,e)=>n+vec(t)[e])}function multVec(n,t){return vec(n).map((n,e)=>n*vec(t)[e])}function divVec(n,t){return vec(n).map((n,e)=>n/vec(t)[e])}function moveTowards(n,t,e){var r=t.x-n.x,c=t.y-n.y;if(mag([r,c])>e){var u=norm([r,c]);r=u[0]*e,c=u[1]*e}return{x:n.x+r,y:n.y+c}}
class World extends PIXI.Container{constructor(){super(),this.bind=createBinder(this),this.sectors={},this.players={},this.bindSocket("joinGame"),this.bindSocket("joinMap"),this.bindSocket("joinSector"),this.bindSocket("addPlayer"),this.bindSocket("removePlayer"),this.bindSocket("movePlayer"),this.bindSocket("updateCharacter")}async loadData(){var e=game.data.maps[game.info.map];console.log(e),e.sectorMap&&(this.sectorMap=await game.loadImage(e.sectorMap))}bindSocket(e){game.on(e,(...a)=>{console.log(e,...a),this.bind(e)(...a)})}screenToWorld(e){var a=game.sector?game.player.worldPos:game.player;return{x:e.x-game.getScreen().width/2+a.x,y:e.y-game.getScreen().height/2+a.y}}worldToScreen(e){var a=game.sector?game.player.worldPos:game.player;return{x:e.x-a.x+game.getScreen().width/2,y:e.y-a.y+game.getScreen().height/2}}getSectorName({x:e,y:a}){return e+"-"+a}getWorldBounds(){var e=game.data.maps[game.info.map].bounds;return new PIXI.Rectangle(e.x*game.info.SECTOR_SIZE,e.y*game.info.SECTOR_SIZE,e.width*game.info.SECTOR_SIZE,e.height*game.info.SECTOR_SIZE)}withinWorld(e){var a=this.getWorldBounds();return rectsIntersect(e,a)}update(e){this.updateInput(e),Object.values(this.sectors).forEach(a=>a.update(e))}updateInput(e){if(game.player){if(game.input.getMouseDown(Mouse.MIDDLE_CLICK))this.x=0,this.y=0;else{var a=game.input.getMouseScroll();game.input.getKey(Keyboard.ALT)?this.x-=a.y*e:(this.x-=a.x*e,this.y-=a.y*e)}(game.inputManager.axisChanged("Down")||game.inputManager.axisChanged("Right"))&&(game.player.moveBy({dx:game.inputManager.getAxis("Right"),dy:game.inputManager.getAxis("Down")}),game.emit("movePlayer",{x:game.player.x,y:game.player.y,dx:game.player.dx,dy:game.player.dy}))}}async joinGame(e){console.log("join game"),game.info.id=e.id,game.info.server=e.server,game.info.SECTOR_SIZE=e.SECTOR_SIZE,await this.joinMap(e)}async joinMap(e){console.log("join map"),game.info.map=e.map,game.data.maps[e.map]=await game.loadFile("data/maps/"+e.map+".json"),await this.joinSector(e)}async joinSector(e){for(var a in console.log("joing sector"),game.info.sector=e.sector,game.info.players=e.players,game.info.sectors=e.sectors,this.sectors)e.sectors.map(e=>e.name).includes(a)||this.removeSector(a);for(var t of e.sectors)if(this.withinWorld(game.getSectorRect(t))){a=this.addSector(t);game.info.sector==t.name&&(game.sector=a,this.removeChild(a),this.addChild(a))}}addSector(e){if(this.sectors[e.name])return this.sectors[e.name].updatePlayers(e.players),this.sectors[e.name];var a=new Sector(e);return this.sectors[e.name]=a,this.addChild(a),a}removeSector(e){this.removeChild(this.sectors[e]),this.sectors[e].destroy({children:!0}),delete this.sectors[e]}addPlayer(e){var a=this.sectors[e.sector]||game.sector;game.info.players.map(e=>e.id).includes(e.id)||game.info.players.push(e),a.addPlayer(e)}removePlayer(e){var a=this.sectors[e.sector]||game.sector;game.info.players.map(e=>e.id).includes(e.id)&&(game.info.player=game.info.players.filter(a=>a!=e.id)),a.removePlayer(e.id)}movePlayer(e){(this.sectors[e.sector]||game.sector).movePlayer(e)}updateCharacter(e){(this.sectors[e.sector]||game.sector).updateCharacter(e)}}
