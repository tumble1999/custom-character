...
var renderer=new THREE.WebGLRenderer;renderer.setSize(window.innerWidth,window.innerHeight),document.body.appendChild(renderer.domElement);var scene=new THREE.Scene,camera=new THREE.PerspectiveCamera(75,window.innerWidth/window.innerHeight,.1,1e3);camera.position.set(0,0,400),camera.lookAt(0,0,0),window.onresize=function(){var e=window.innerWidth,n=window.innerHeight;renderer&&(renderer.setSize(e,n),camera.aspect=e/n,camera.updateProjectionMatrix())};var controls=new THREE.OrbitControls(camera,renderer.domElement),spotLight=new THREE.SpotLight(16777215);spotLight.position.set(600,600,600),scene.add(spotLight);var drawingSpace=new DrawSpace(640,480);function screenToWorld(e,n,r=0){var o=new THREE.Vector3,a=new THREE.Vector3;o.set(e/window.innerWidth*2-1,-n/window.innerHeight*2+1,.5),o.unproject(camera),o.sub(camera.position).normalize();var i=(r-camera.position.z)/o.z;return a.copy(camera.position).add(o.multiplyScalar(i)),a}function update(){requestAnimationFrame(update),controls.update(),renderer.render(scene,camera)}window.onmousedown=function(e){var n=screenToWorld(e.offsetX,e.offsetY);drawingSpace.startDrawing(n.x,n.y)},window.onmouseup=function(e){var n=screenToWorld(e.offsetX,e.offsetY);drawingSpace.stopDrawing(n.x,n.y)},window.onmousemove=function(e){var n=screenToWorld(e.offsetX,e.offsetY);drawingSpace.draw(n.x,n.y)},scene.add(drawingSpace.createMesh()),update();
