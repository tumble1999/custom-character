...
class Sector extends PIXI.Container{constructor(e){super(),this.info=e,this.id=e.id||e.name,this.name=e.name,this.x=e.x,this.y=e.y,this.players={},game.data.maps[game.info.map]&&(this.data=game.data.maps[game.info.map][this.id],this.loadData()),this.background=PIXI.Sprite.from(PIXI.Texture.WHITE),this.background.tint=65536*e.x+256*e.y+255,this.background.width=this.background.height=game.info.SECTOR_SIZE,this.addChild(this.background),this.updatePlayers()}async loadData(){this.collision=await game.loadFile(this.data.collisionMap)}update(e){if(game.player){var i=game.world.worldToScreen(this.info);this.x=i.x,this.y=i.y,Object.values(this.players).forEach(i=>i.update(e))}}worldToSector(e={x:0,y:0}){return{x:e.x-this.info.x,y:e.y-this.info.y}}sectorToWorld(e={x:0,y:0}){return{x:e.x+this.info.x,y:e.y+this.info.y}}updatePlayers(){var e=this,i=game.info.players.filter(i=>e.name==i.sector.x+"-"+i.sector.y);for(var a in this.players)i.map(e=>e.id).includes(a)||this.removePlayer(a);for(var t of i){a=this.addPlayer(t);game.info.id==t.id&&(game.player=a,this.removeChild(a),this.addChild(a))}}addPlayer(e){if(e.textureBlob=blobFromBytes(e.textureBlob),this.players[e.id])return this.players[e.id].updateInfo(e),this.players[e.id];var i=new Player(e);return this.players[e.id]=i,this.addChild(i),i}removePlayer(e){this.removeChild(this.players[e]),this.players[e].destroy({children:!0}),delete this.players[e]}movePlayer(e){this.players[e.id].moveTo(e)}destroy(e){super.destroy(e),console.log(this.name,"destroyed")}}
