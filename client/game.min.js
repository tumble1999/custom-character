...
class Game{constructor(){this.bind=createBinder(this),this.touchHandler=new TouchHandler,this.setupInput(),this.loader=new PIXI.Loader,this.data={maps:{},characters:{},input:{}},this.app=new App(window.innerWidth,window.innerHeight,"#000000"),this.info={},document.body.appendChild(this.app.domElement()),this.app.onUpdate(this.bind("update")),window.addEventListener("mousedown",this.bind("mouseDown")),window.addEventListener("mouseup",this.bind("mouseUp")),window.addEventListener("mousemove",this.bind("mouseMove")),window.addEventListener("resize",this.bind("windowResize")),window.addEventListener("mousewheel",e=>{e.altKey?game.world.x-=e.deltaY:game.world.y-=e.deltaY})}setupInput(){this.input=new Input,this.inputManager=new InputManager,this.inputManager.onAxis("Down",(function(){var e=game.input.getKey(Keyboard.UP_ARROW)?1:0;return(game.input.getKey(Keyboard.DOWN_ARROW)?1:0)-e}),(function(){return game.input.getKeyDown(Keyboard.UP_ARROW)||game.input.getKeyDown(Keyboard.DOWN_ARROW)}),(function(){return game.input.getKeyUp(Keyboard.UP_ARROW)||game.input.getKeyUp(Keyboard.DOWN_ARROW)})),this.inputManager.onAxis("Right",(function(){var e=game.input.getKey(Keyboard.LEFT_ARROW)?1:0;return(game.input.getKey(Keyboard.RIGHT_ARROW)?1:0)-e}),(function(){return game.input.getKeyDown(Keyboard.LEFT_ARROW)||game.input.getKeyDown(Keyboard.RIGHT_ARROW)}),(function(){return game.input.getKeyUp(Keyboard.LEFT_ARROW)||game.input.getKeyUp(Keyboard.RIGHT_ARROW)}))}getScreen(){return this.app.screen}update(e){this.input&&this.input.update(),this.dialogue&&this.dialogue.update&&this.dialogue.update(e),this.world&&this.world.update&&this.world.update(e)}async loadFile(e){var t=this;return new Promise((i,o)=>{t.loader.add(e),t.loader.load((t,o)=>{i(o[e].data),console.log("File Loaded",e)})})}startGame(e){this.dialogue&&(this.dialogue.destroy(),delete this.dialogue);var t=io(e);this.socket=t,this.dialogue=new BorderText("Connecting To Server...",{fontSize:60}),centerTo(this.dialogue,this.getScreen()),this.app.addChild(this.dialogue),t.on("connect",this.bind("connectedToServer"))}connectedToServer(){this.world&&(this.world.destroy({children:!0}),delete this.player,delete this.sector,this.info={},delete this.world),this.dialogue&&(this.dialogue.destroy(),delete this.dialogue),console.log("Connected To Server"),this.dialogue=new EnterTextScreen("Enter Username",this.bind("login")),this.app.addChild(this.dialogue)}async createCharacter(){console.log("Under Construction")}async cleanDialogue(){var e=!0;this.dialogue.textInput&&(this.info.name=this.dialogue.textInput.text,e=!1),this.dialogue.drawingSpace&&(this.info.textureBlob=await this.dialogue.drawingSpace.createBlob()),this.dialogue.destroy({children:e}),delete this.dialogue}async login(){this.dialogue&&this.cleanDialogue(),this.data.characters.default=await this.loadFile("media/default.png"),this.world=new World,this.app.addChild(this.world),this.info&&this.emit("joinGame",this.info)}emit(...e){this.socket&&this.socket.emit(...e)}on(...e){this.socket&&this.socket.on(...e)}mouseDown(e){if(2==e.which)return game.world.x=0,void(game.world.y=0);this.dialogue&&this.dialogue.mouseDown&&this.dialogue.mouseDown(e),this.world&&this.world.mouseDown&&this.world.mouseDown(e)}mouseMove(e){this.dialogue&&this.dialogue.mouseMove&&this.dialogue.mouseMove(e)}mouseUp(e){this.dialogue&&this.dialogue.mouseUp&&this.dialogue.mouseUp(e)}windowResize(e){this.app.resizeTo=document.body,this.app.resize()}}
PIXI.settings.SCALE_MODE=PIXI.SCALE_MODES.NEAREST;
class Player extends PIXI.Container{constructor(t){super(),this.id=t.id,this.name=t.name,this.destination={x:this.x,y:this.y},this.moving=!1,this.dx=0,this.dy=0,this.abilities=[],this.abilities.length=4,this.animation={yMax:6,speed:.013},this.character=new PIXI.Sprite(PIXI.Texture.from(game.data.characters.default)),this.addChild(this.character),this.nicknameSprite=new BorderText(this.name),this.addChild(this.nicknameSprite),this.updateInfo(t)}get worldPos(){return{x:this.sector.x*game.info.SECTOR_SIZE+this.x,y:this.sector.y*game.info.SECTOR_SIZE+this.y}}async updateInfo(t){this.info=t,t.textureBlob&&(this.character.texture=PIXI.Texture.from(URL.createObjectURL(t.textureBlob)));var i=subVec({x:this.x,y:this.y},this.destination);this.map=t.map,this.x=t.x+i[0],this.y=t.y+i[1],this.sector=t.sector,this.speed=t.speed}updateFloatAnimation(){var t=this.animation.speed;this.moving&&(t*=1.5*this.speed);var i=performance.now()*t/this.animation.yMax;this.character.y=(Math.sin(i)-1)*this.animation.yMax}update(){this.moving=0!=this.dx||0!=this.dy,this.moving&&(this.x+=this.dx*this.speed,this.y+=this.dy*this.speed),centerTo(this.nicknameSprite,this),this.nicknameSprite.y=0,this.updateFloatAnimation()}moveBy(t){t.x&&(this.x=t.x),t.y&&(this.y=t.y),isNaN(t.dx)||(this.dx=t.dx),isNaN(t.dy)||(this.dy=t.dy)}moveTo(t){this.info.x=t.x,this.info.y=t.y;this.destination={x:t.x-this.width/2,y:t.y-3/4*this.height}}destroy(t){super.destroy(t),console.log(this.name,"destroyed")}}
...
class InputManager{constructor(){this.axis={}}onAxis(i,s,a,t){this.axis[i]={down:a,value:s,up:t}}getAxis(i){if(!this.axis[i])return 0;var s=this.axis[i].value();return isNaN(s)?0:s}axisDown(i){if(!this.axis[i])return!1;var s=this.axis[i].down();return!!isBoolean(s)&&s}axisUp(i){if(!this.axis[i])return!1;var s=this.axis[i].up();return!!isBoolean(s)&&s}axisChanged(i){return this.axisDown(i)||this.axisUp(i)}axisNotZero(i){return 0!=this.getAxis(i)}}
