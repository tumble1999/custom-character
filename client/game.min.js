...
class Room extends PIXI.Container{constructor(e){super(),this.info=e,this.name=e.name,this.x=e.x,this.y=e.y,this.players={},this.background=PIXI.Sprite.from(PIXI.Texture.WHITE),this.background.tint=65536*e.x+256*e.y+255,this.background.width=this.background.height=4096,this.addChild(this.background),this.updatePlayers(e.players)}update(e){var r=game.world.worldToScreen(this.info);this.x=r.x,this.y=r.y,Object.values(this.players).forEach(r=>r.update(e))}worldToRoom(e){return{x:e.x-this.info.x,y:e.y-this.info.y}}roomToWorld(e){return{x:e.x+this.info.x,y:e.y+this.info.y}}updatePlayers(e){for(var r in this.players)e.map(e=>e.id).includes(r)||this.removePlayer(r);for(var t of e){r=this.addPlayer(t);game.playerInfo.id==t.id&&(game.player=r)}}addPlayer(e){if(this.players[e.id||e.name])return this.players[e.id||e.name].updateInfo(e),this.players[e.id||e.name];e.textureBlob=blobFromBytes(e.textureBlob);var r=this.worldToRoom(e);e.x=r.x,e.y=r.y;var t=new Player(e);return this.players[e.id||e.name]=t,this.addChild(t),t}removePlayer(e){this.players[e].destroy({children:!0}),delete this.players[e]}movePlayer(e){var r=this.worldToRoom(e);this.players[e.id].moveTo(r)}}
...
class World extends PIXI.Container{constructor(){super(),this.bind=createBinder(this),this.rooms={},this.bindSocket("joinGame"),this.bindSocket("addPlayer"),this.bindSocket("removePlayer"),this.bindSocket("movePlayer"),this.bindSocket("updateRooms")}bindSocket(e){game.on(e,(...o)=>{console.log(e,...o),this.bind(e)(...o)})}screenToWorld(e){var o=game.room?game.room.roomToWorld(game.player):game.player;return{x:e.x-game.getScreen().width/2+o.x,y:e.y-game.getScreen().height/2+o.y}}worldToScreen(e){var o=game.room?game.room.roomToWorld(game.player):game.player;return{x:e.x-o.x+game.getScreen().width/2,y:e.y-o.y+game.getScreen().height/2}}update(e){Object.values(this.rooms).forEach(o=>o.update(e))}mouseDown(e){var o=this.screenToWorld({x:e.pageX,y:e.pageY});game.emit("movePlayer",o),o=game.room.worldToRoom(o),game.player.moveTo(o)}joinGame(e){game.playerInfo.id=e.id,game.playerInfo.room=e.room,this.updateRooms(e.rooms)}addRoom(e){if(this.rooms[e.name])return this.rooms[e.name].updatePlayers(e.players),this.rooms[e.name];var o=new Room(e);return this.rooms[e.name]=o,this.addChild(o),o}movePlayer(e){this.rooms[e.room].movePlayer(e)}updateRooms(e){for(var o in game.playerInfo.room=e[0].name,this.rooms)e.map(e=>e.name).includes(o)||(this.rooms[o].destroy({children:!0}),delete this.rooms[o]);for(var r of e){o=this.addRoom(r);game.playerInfo.room==r.name&&(game.room=o)}}}
